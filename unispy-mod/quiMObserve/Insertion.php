...
//  preg_match_all("#((\d{2})\-(\d{2}) (\d{2}):(\d{2}):(\d{2})\s+Contrôle\saérospatial(.*?)Probabilité\sde\sdestruction\sde\sla\sflotte\sd(.*?)espionnage\s:\s(\d+)\s%)#si", $pub_espionage,$matches);
  preg_match_all("#((\w{3})\s(\d{2})\s(\w{3})\s(\d{2}):(\d{2}):(\d{2})\s+Tour\sde\scontrôle\s+Activité\sespionnage(.*?)Probabilité\sde\sdestruction\sde\sla\sflotte\sespionnage\s:\s+(\d+)\s%)#si", $pub_espionage,$matches);
  
  foreach ($matches[1] as $key => $val){
		//on verifie si le mois en cours est inferieur au mois du sondage (pour eviter le bug du changement d année)
		$year = date('Y');
		$month = array_search($matches[4][$key], array(1 => 'Jan',2 => 'Fev', 3 => 'Mar', 4 => 'Apr', 5 => 'May', 6 => 'Jun', 7 => 'Jul', 8 => 'Aug', 9 => 'Sep', 10 => 'Oct', 11 => 'Nov', 12 => 'Dec'));
		if(date('m') < $month){
			$year -= 1; 
		}
		$timestamp = mktime($matches[5][$key],$matches[6][$key],$matches[7][$key],$month,$matches[3][$key],$year);
