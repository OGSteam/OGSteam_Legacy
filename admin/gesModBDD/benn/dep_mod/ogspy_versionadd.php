...
// Create page object
$ogspy_version_add = new cogspy_version_add();
$Page =& $ogspy_version_add;

// Page init
$ogspy_version_add->Page_Init();

// Page main
$ogspy_version_add->Page_Main();
?>
...
// Create page object
var ogspy_version_add = new ew_Page("ogspy_version_add");

// page properties
ogspy_version_add.PageID = "add"; // page ID
ogspy_version_add.FormID = "fogspy_versionadd"; // form ID
var EW_PAGE_ID = ogspy_version_add.PageID; // for backward compatibility

// extend page with ValidateForm function
ogspy_version_add.ValidateForm = function(fobj) {
...
ogspy_version_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
ogspy_version_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
ogspy_version_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
ogspy_version_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ogspy_version->TableCaption() ?><br><br>
<a href="<?php echo $ogspy_version->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$ogspy_version_add->ShowMessage();
?>
<form name="fogspy_versionadd" id="fogspy_versionadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ogspy_version_add.ValidateForm(this);">
<p>
...
<?php
$ogspy_version_add->Page_Terminate();
?>
<?php
...
class cogspy_version_add {
	...
	// Page object name
	var $PageObjName = 'ogspy_version_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $ogspy_version->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$ogspy_version->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $ogspy_version->CurrentAction = "C"; // Copy record
		  } else {
		    $ogspy_version->CurrentAction = "I"; // Display blank record
		...
		$ogspy_version->RowType = EW_ROWTYPE_ADD;  // Render add type
