 ...
 */
function galaxy_add_system($galaxy, $system, $row, $moon, $name, $ally, $player,
    $status, $timestamp, $mode_ogs = false)
{
    global $db, $user_data;
    ...
    if (!check_var($row, "Num") || $row < 1 && $row > 15)
        return false;

    //Contrôle si l'update entraine des modifications
    $request = "select ally, player, last_update from " . TABLE_UNIVERSE .
    ...
    if ($last_update < $timestamp) {
        ...
        if (preg_match("#i#", $values))
            $status .= "i";
        if (preg_match("#b#", $values))
            $status .= "b";
        if (preg_match("#v#", $values))
            $status .= "v";
        if (preg_match("#d#", $values))
            $status .= "d";
        if (preg_match("#f#", $values))
            $status .= "f";
        if (preg_match("#I#", $values))
            $status .= "I";

        //Mise à jour
        $request = "update " . TABLE_UNIVERSE . " set ally = '" . $db->
            sql_escape_string($ally) . "', player = '" . $db->sql_escape_string($player) .
            "', moon = '" . $moon . "', name = '" . $db->sql_escape_string($name) .
            "', status = '" . $status . "', last_update = '" . $timestamp .
            "', last_update_user_id = " . $user_data["user_id"];
        $request .= " where galaxy = '" . $galaxy . "' and system = " . $system .
            " and row = '" . $row . "'";
        $db->sql_query($request) or $error = true;
        if ($db->sql_affectedrows() == 0) {
            $request = "insert ignore into " . TABLE_UNIVERSE .
                " (galaxy, system, row, moon, name, ally, player, status, last_update, last_update_user_id)";
            $request .= " values ('" . $galaxy . "', " . $system . ", '" . $row . "', '" . $moon .
                "', '" . $db->sql_escape_string($name) . "', '" . $db->sql_escape_string($ally) .
                "', '" . $db->sql_escape_string($player) . "', '" . $status . "', " . $timestamp .
                ", " . $user_data["user_id"] . ")";
            $db->sql_query($request) or $error = true;
        }
        if ($moon == 0) {
            $request = "update " . TABLE_UNIVERSE .
                " set phalanx = 0, gate = '0', last_update_moon = 0 where galaxy = '" . $galaxy .
                "' and system = " . $system . " and row = '" . $row . "'";
            $db->sql_query($request) or $error = true;
        }
    ...
    $gate)
{
    ...
    $spy_added = array();
    $request = "insert into " . TABLE_SPY .
        " (spy_galaxy, spy_system, spy_row, sender_id, datadate, rawdata)" . " values ('$galaxy', '$system', '$row', " .
        $user_data["user_id"] . ", '$timestamp', '" . $db->sql_escape_string($report) .
        "')";
    if ($db->sql_query($request, false)) {
        ...
        $moon = 0;
        if ((strcmp($planet, "lune") == 0 || strpos($planet, " (Lune)") != 0) && !
            preg_match("#Mine#", $report))
            $moon = 1;

        $request = "select last_update, last_update_moon from " . TABLE_UNIVERSE .
            " where galaxy = $galaxy and system = $system and row = $row";
        $result = $db->sql_query($request);
        list($last_update, $last_update_moon) = $db->sql_fetch_row($result);

        if ($timestamp > $last_update) {
            $request = "update " . TABLE_UNIVERSE . " set last_update = '" . $timestamp .
                "', last_update_user_id = " . $user_data["user_id"];
            if (!$moon)
                $request .= ", name = '" . $db->sql_escape_string($planet) . "'";
            $request .= " where galaxy = '" . $galaxy . "' and system = " . $system .
                " and row = '" . $row . "'";
            $db->sql_query($request);
            ...
            if ($db->sql_affectedrows() == 0) {
                $request = "insert ignore into " . TABLE_UNIVERSE .
                    " (galaxy, system, row, name, last_update, last_update_user_id)";
                $request .= " values ('" . $galaxy . "', " . $system . ", '" . $row . "', '" . $db->
                    sql_escape_string($planet) . "', " . $timestamp . ", " . $user_data["user_id"] .
                    ")";
                $db->sql_query($request);
            }
        }
        ...
        if ($timestamp > $last_update_moon && $moon) {
            $request = "update " . TABLE_UNIVERSE . " set moon = '1', last_update_moon = " .
                $timestamp . ", phalanx = " . $phalanx . ", gate = '" . $gate .
                "' where galaxy = '" . $galaxy . "' and system = " . $system . " and row = '" .
                $row . "'";
            $db->sql_query($request);
        }
...
{
    $return = array('planet_name' => '', 'coordinates' => '', 'dateRE' => '', 'M' =>
        -1, 'C' => -1, 'D' => -1, 'CES' => -1, 'CEF' => -1, 'UdR' => -1, 'UdN' => -1,
        'CSp' => -1, 'HM' => -1, 'HC' => -1, 'HD' => -1, 'Lab' => -1, 'Ter' => -1, 'DdR' =>
        -1, 'Silo' => -1, 'BaLu' => -1, 'Pha' => -1, 'PoSa' => -1, 'LM' => -1, 'LLE' =>
        -1, 'LLO' => -1, 'CG' => -1, 'AI' => -1, 'LP' => -1, 'PB' => -1, 'GB' => -1,
        'MIC' => -1, 'MIP' => -1, 'PT' => -1, 'GT' => -1, 'CLE' => -1, 'CLO' => -1, 'CR' =>
        -1, 'VB' => -1, 'VC' => -1, 'REC' => -1, 'SE' => -1, 'BMD' => -1, 'SAT' => -1,
        'DST' => -1, 'EDLM' => -1, 'TRA' => -1, 'Esp' => -1, 'Ordi' => -1, 'Armes' => -
        1, 'Bouclier' => -1, 'Protection' => -1, 'NRJ' => -1, 'HYP' => -1, 'RC' => -1,
        'RI' => -1, 'PH' => -1, 'Laser' => -1, 'Ions' => -1, 'Plasma' => -1, 'RRI' => -
        1, 'Graviton' => -1, 'Astrophysique' => -1, 'metal' => -1, 'cristal' => -1,
        'deuterium' => -1, 'energie' => -1, 'activite' => -1, 'proba' => 0);
    $rawRE = stripslashes(ereg_replace("\n|\r|\r\n", " \t", $rawRE));
    while (preg_match('/(\d+)\.(\d+)/', $rawRE))
        $rawRE = preg_replace('/(\d+)\.(\d+)/', "$1$2", $rawRE);
    $match = false;
    $idx = 0;
    $regExp = array('/.*Flotte(.*)D.fense(.*)B.timents(.*)Recherche(.*)/',
        '/.*Flotte(.*)D.fense(.*)B.timents(.*)/', '/.*Flotte(.*)D.fense(.*)/',
        '/.*Flotte(.*)/', '/.*/');

    if (preg_match('/Mati.res premi.res sur (.*?) \[(\d+:\d+:\d+)\](?: \(Joueur \'.*?\'\))?[\s]*le (\d+-\d+ \d+:\d+:\d+)\s*M.tal:(\s)*(\d+)\s*Cristal:(\s)*(\d+)\s*Deut.rium:(\s)*(\d+)\s*Energie:(\s)*(\d+).*Probabilit. de destruction de la flotte d.espionnage( ?):([0-9]{1,3})%/',
        $rawRE, $reg)) {
        ...
        if (isset($reg[1]))
            $total_flotte = trim($reg[1]);
        if (isset($reg[2]))
            $total_defense = trim($reg[2]);
        if (isset($reg[3]))
            $total_build = trim($reg[3]);
        if (isset($reg[4]))
            $total_search = trim($reg[4]);
        ...
        if (isset($total_search)) {
            ...
            if (isset($reg[1]))
                $return['Bouclier'] = $reg[1];
            else
                $return['Bouclier'] = 0;
            preg_match('/Technologie Protection des vaisseaux spatiaux\s+(\d+)/', $total_search,
                $reg);
            if (isset($reg[1]))
                $return['Protection'] = $reg[1];
            else
                $return['Protection'] = 0;
            ...
            if (isset($reg[1]))
                $return['Graviton'] = $reg[1];
            else
                $return['Graviton'] = 0;
            preg_match('/Technologie Astrophysique\s+(\d+)/', $total_search, $reg);
            if (isset($reg[1]))
                $return['Astrophysique'] = $reg[1];
            else
                $return['Astrophysique'] = 0;
        }
        return ($return);
    } else
        return false;
...
function insert_RE($rawRE, $sender_id = -1)
{
    ...
    if ($parsedRE = parseRE($rawRE)) {
        if ($parsedRE['PT'] == -1)
            $limit = 0;
        elseif ($parsedRE['LM'] == -1)
            $limit = 1;
        elseif ($parsedRE['M'] == -1)
            $limit = 2;
        elseif ($parsedRE['Esp'] == -1)
            $limit = 3;
        else
            $limit = 4;
        switch ($limit) {
            case 0:
                ...
                $values = array($parsedRE['planet_name'], $parsedRE['coordinates'], $parsedRE['metal'],
                    $parsedRE['cristal'], $parsedRE['deuterium'], $parsedRE['energie'], $parsedRE['activite'],
            ...
            case 1:
                ...
                $values = array($parsedRE['planet_name'], $parsedRE['coordinates'], $parsedRE['metal'],
                    $parsedRE['cristal'], $parsedRE['deuterium'], $parsedRE['energie'], $parsedRE['activite'],
                    $parsedRE['PT'], $parsedRE['GT'], $parsedRE['CLE'], $parsedRE['CLO'], $parsedRE['CR'],
                    $parsedRE['VB'], $parsedRE['VC'], $parsedRE['REC'], $parsedRE['SE'], $parsedRE['BMD'],
                    $parsedRE['DST'], $parsedRE['EDLM'], $parsedRE['SAT'], $parsedRE['TRA'], $parsedRE['dateRE'],
            ...
            case 2:
                ...
                $values = array($parsedRE['planet_name'], $parsedRE['coordinates'], $parsedRE['metal'],
                    $parsedRE['cristal'], $parsedRE['deuterium'], $parsedRE['energie'], $parsedRE['activite'],
                    $parsedRE['LM'], $parsedRE['LLE'], $parsedRE['LLO'], $parsedRE['CG'], $parsedRE['AI'],
                    $parsedRE['LP'], $parsedRE['PB'], $parsedRE['GB'], $parsedRE['MIC'], $parsedRE['MIP'],
                    $parsedRE['PT'], $parsedRE['GT'], $parsedRE['CLE'], $parsedRE['CLO'], $parsedRE['CR'],
                    $parsedRE['VB'], $parsedRE['VC'], $parsedRE['REC'], $parsedRE['SE'], $parsedRE['BMD'],
                    $parsedRE['DST'], $parsedRE['EDLM'], $parsedRE['SAT'], $parsedRE['TRA'], $parsedRE['dateRE'],
                    $parsedRE['proba'], $user_id);
            ...
            case 3:
                ...
                $values = array($parsedRE['planet_name'], $parsedRE['coordinates'], $parsedRE['metal'],
                    $parsedRE['cristal'], $parsedRE['deuterium'], $parsedRE['energie'], $parsedRE['activite'],
                    $parsedRE['M'], $parsedRE['C'], $parsedRE['D'], $parsedRE['CES'], $parsedRE['CEF'],
                    $parsedRE['UdR'], $parsedRE['UdN'], $parsedRE['CSp'], $parsedRE['HM'], $parsedRE['HC'],
                    $parsedRE['HD'], $parsedRE['Lab'], $parsedRE['Ter'], $parsedRE['DdR'], $parsedRE['Silo'],
                    $parsedRE['BaLu'], $parsedRE['Pha'], $parsedRE['PoSa'], $parsedRE['LM'], $parsedRE['LLE'],
                    $parsedRE['LLO'], $parsedRE['CG'], $parsedRE['AI'], $parsedRE['LP'], $parsedRE['PB'],
                    $parsedRE['GB'], $parsedRE['MIC'], $parsedRE['MIP'], $parsedRE['PT'], $parsedRE['GT'],
                    $parsedRE['CLE'], $parsedRE['CLO'], $parsedRE['CR'], $parsedRE['VB'], $parsedRE['VC'],
                    $parsedRE['REC'], $parsedRE['SE'], $parsedRE['BMD'], $parsedRE['DST'], $parsedRE['EDLM'],
                    $parsedRE['SAT'], $parsedRE['TRA'], $parsedRE['dateRE'], $parsedRE['proba'], $user_id);
                break;
            case 4:
                $fields = 'planet_name, coordinates, metal, cristal, deuterium, energie, activite, M, C, D, CES, CEF, UdR, UdN, CSp, HM, HC, HD, 
					Lab, Ter, DdR, Silo, BaLu, Pha, PoSa, LM, LLE, LLO, CG, AI, LP, PB, GB, MIC, MIP, PT, GT, CLE, CLO, CR, VB, VC, REC, SE, BMD, 
                ...
                $values = array($parsedRE['planet_name'], $parsedRE['coordinates'], $parsedRE['metal'],
                    ...
                    $parsedRE['CLE'], $parsedRE['CLO'], $parsedRE['CR'], $parsedRE['VB'], $parsedRE['VC'],
                    $parsedRE['REC'], $parsedRE['SE'], $parsedRE['BMD'], $parsedRE['DST'], $parsedRE['EDLM'],
                    $parsedRE['SAT'], $parsedRE['TRA'], $parsedRE['Esp'], $parsedRE['Ordi'], $parsedRE['Armes'],
                    $parsedRE['Bouclier'], $parsedRE['Protection'], $parsedRE['NRJ'], $parsedRE['Hyp'],
                    $parsedRE['RC'], $parsedRE['RI'], $parsedRE['PH'], $parsedRE['Laser'], $parsedRE['Ions'],
                    $parsedRE['Plasma'], $parsedRE['RRI'], $parsedRE['Graviton'], $parsedRE['Astrophysique'],
                    $parsedRE['dateRE'], $parsedRE['proba'], $user_id);
                break;
        }
        $query = 'SELECT id_spy FROM ' . TABLE_PARSEDSPY . ' WHERE coordinates = "' . $parsedRE['coordinates'] .
            '" AND dateRE = ' . $parsedRE['dateRE'];
        $res = $db->sql_query($query);
        if ($db->sql_numrows($res) == 0) {
            $query = 'INSERT INTO ' . TABLE_PARSEDSPY . '(' . $fields . ') VALUES ("' . join('", "',
                $values) . '")';
            if ($db->sql_query($query)) {
                list($galaxy, $system, $row) = explode(':', $parsedRE['coordinates']);
                $spy_added[] = 'galaxy=' . $galaxy . '&system=' . $system . '&row=' . $row;
                if (preg_match('/ \(Lune\)/', $parsedRE['planet_name']) || ($parsedRE['planet_name'] ==
                    'lune' && !preg_match('/Mine/', $rawRE))) {
                    $request = 'SELECT last_update_moon from ' . TABLE_UNIVERSE . ' where galaxy = ' .
                        $galaxy . ' and 
						system = ' . $system . ' and row = ' . $row;
                    $result = $db->sql_query($request);
                    list($last_update_moon) = $db->sql_fetch_row($result);
                    if ($parsedRE['dateRE'] > $last_update_moon) {
                        $request = 'UPDATE ' . TABLE_UNIVERSE . ' SET last_update_user_id = ' . $user_data['user_id'] .
                            ', moon = "1", 
							last_update_moon = ' . $parsedRE['dateRE'] . ', last_update = ' . $parsedRE['dateRE'] .
                            ', phalanx = ' . $parsedRE['Pha'] . ', gate = "' . $parsedRE['PoSa'] .
                            '" WHERE galaxy = ' . $galaxy . ' AND system = ' . $system . ' AND 
							row = ' . $row;
                        $db->sql_query($request);
                    }
                } else {
                    $request = 'SELECT last_update from ' . TABLE_UNIVERSE . ' where galaxy = ' . $galaxy .
                        ' and system = ' . $system . ' and 
						row = ' . $row;
                    $result = $db->sql_query($request);
                    list($last_update) = $db->sql_fetch_row($result);
                    if ($parsedRE['dateRE'] > $last_update) {
                        $request = 'UPDATE ' . TABLE_UNIVERSE . ' SET last_update = ' . $parsedRE['dateRE'] .
                            ', last_update_user_id = ' . $user_data['user_id'] . ', name = "' . $db->
                            sql_escape_string($parsedRE['planet_name']) . '" WHERE galaxy = ' . $galaxy .
                            ' AND system = ' . $system . ' AND row = ' . $row;
                        $db->sql_query($request);
                    }
                }
