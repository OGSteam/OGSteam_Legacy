...
function user_set_general($user_id, $username = null, $user_password = null, $user_lastvisit = null, $user_galaxy = null, $user_system = null, $user_skin = null, $disable_ip_check = null) {
	global $db, $user_data;
	...
	if (!empty($username))
	$update .= "username = '".mysql_real_escape_string($username)."'";
...
function user_set_stat($planet_added_web = null, $planet_added_ogs = null, $search = null, $spy_added_web = null, $spy_added_ogs = null, $rank_added_web = null, $rank_added_ogs = null, $planet_exported = null, $spy_exported = null, $rank_exported = null) {
	...
	//Statistiques envoi systèmes solaires et rapport d'espionnage
...
function user_create() {
	...
	$password = password_generator();
	//$request = "select user_id from ".TABLE_USER." where username = '".mysql_real_escape_string($pub_pseudo)."'";
...
function user_getfavorites_spy() {
	...
	while (list($spy_id, $spy_galaxy, $spy_system, $spy_row, $datadate, $sender_id, $moon, $ally, $player, $status) = $db->sql_fetch_row($result)) {
		...
		$result_2 = $db->sql_query($request);
		list($username) = $db->sql_fetch_row($result_2);
		$favorite[$spy_id] = array("spy_id" => $spy_id, "spy_galaxy" => $spy_galaxy, "spy_system" => $spy_system, "spy_row" => $spy_row, "player" => $player, "ally" => $ally, "moon" => $moon, "status" => $status, "datadate" => $datadate, "poster" => $username);
	}
...
function usergroup_setauth() {
	...
	!check_var($pub_ogs_get_system, "Num") || !check_var($pub_ogs_set_spy, "Num") || !check_var($pub_ogs_get_spy, "Num") ||
...
function usergroup_member($group_id) {
	...
	$request = "select user_id, username from ".TABLE_USER_GROUP."";
	$request .= " where group_id = ".intval($group_id);
	$request .= " order by username";
	$result = $db->sql_query($request);
	while ($row = $db->sql_fetch_assoc()) {
