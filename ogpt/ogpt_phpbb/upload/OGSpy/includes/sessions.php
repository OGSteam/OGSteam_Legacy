...
function session_set_user_data() {
	...
	if (preg_match("#^0.300([b-f])?$#", $server_config["version"])) {
		$request = "select user_id, username, user_admin, user_galaxy, user_system, user_skin";
	}
	elseif (preg_match("#^0.301(b)?$#", $server_config["version"])) {
		$request = "select user_id, username, user_admin, user_galaxy, user_system, user_skin, session_lastvisit";
	}
	elseif (preg_match("#^3.02(b)?$#", $server_config["version"])) {
		$request = "select user_id, username, user_admin, user_coadmin, user_galaxy, user_system, user_skin, session_lastvisit, user_stat_name, ";
...
function session_whois_online() {
	...
	$request = "select username, session_start, session_expire, session_ip, session_ogs";
	...
	while (list($username, $session_start, $session_expire, $session_ip, $session_ogs) = $db->sql_fetch_row($result)) {
		$time_lastactivity = $session_expire - $cookie_time*60;
		$session_ip = decode_ip($session_ip);

		if (is_null($username)) {
			$username = "Visiteur non identifié";
			$guests[] = array("user" => $username, "time_start" => $session_start, "time_lastactivity" => $time_lastactivity, "ip" => $session_ip, "ogs" => 0);
		}
		else {
			$members[] = array("user" => $username, "time_start" => $session_start, "time_lastactivity" => $time_lastactivity, "ip" => $session_ip, "ogs" => $session_ogs);
		}
	}
