...
foreach ($reports as $v) {
	...
	$sender = $v["sender"];
	$report = nl2br($v["data"]);
	$report = str_replace("Matières premières", "<>Matières premières", $report);
	$report = str_replace("Flotte", "<>Flotte", $report);
	$report = str_replace("Défense", "<>Défense", $report);
	$report = str_replace("Bâtiments", "<>Bâtiments", $report);
	$report = str_replace("Recherche", "<>Recherche", $report);
	$report = str_replace("Probabilité", "<->Probabilité", $report);
...
//echo "\t\t\t"."<td class='c' width='150' onclick=\"window.location = 'index.php?action=about&subaction=ogsteam';\">";
	...
	echo "<center><b>Rapport d'espionnage envoyé par ".$sender."</b></center>"."\n";
