...
*************************************************************************/
class acp_portal_include_block_index
    {
    var $u_action;
    function main($id, $imod_portal_blocke)
    {
        ...
        $this->tpl_name = 'acp_portal_include_block_index';
        $this->page_title = 'acp_portal_include_block_index';
        $error = $notify = array();
        ...
        $table = PORTAL_BLOCK_INCLUDES_ORDER_INDEX_TABLE;		       
		...
		case 'saveperm':
				...
				{
				    $sql = 'UPDATE ' . PORTAL_BLOCK_INCLUDES_ORDER_INDEX_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $sql_ary) . " WHERE portal_block_includes_id = $portal_block_includes_id";
				    $message = $user->lang['PORTAL_BLOCK_VIEW_UPDATED'];
				}
				else
				{
				     $sql = 'INSERT INTO ' . PORTAL_BLOCK_INCLUDES_ORDER_INDEX_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
				     $message = $user->lang['PORTAL_BLOCK_VIEW_UPDATED'];
				}
				$db->sql_query($sql);
        ...
        case 'save':
            ...
            case 'editperm':
                ...
                $sqlperm = 'SELECT *
                    FROM ' . PORTAL_BLOCK_INCLUDES_ORDER_INDEX_TABLE . '
            ...
            case 'delete':
                if ($portal_block_includes_id)
                {   
                    $sql = 'DELETE FROM ' . PORTAL_BLOCK_INCLUDES_ORDER_INDEX_TABLE . "
