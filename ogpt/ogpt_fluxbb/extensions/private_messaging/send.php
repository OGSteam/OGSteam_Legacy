...
if (isset($_POST['form_sent']))
{
	...
	else if ($forum_config['p_subject_all_caps'] == '0' && utf8_strtoupper($subject) == $subject && !$forum_user['is_admmod'])
		$subject = utf8_ucwords(utf8_strtolower($subject));

	$send_to = forum_trim($_POST['req_to']);
	...
	else if ($forum_config['p_message_all_caps'] == '0' && utf8_strtoupper($message) == $message && !$forum_user['is_admmod'])
	...
	$query = array(
		'SELECT'	=> 'u.id',
		'FROM'		=> 'users AS u',
		'WHERE'		=> 'u.username=\''.$forum_db->escape($send_to).'\'',
	);
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		// Send message
		$query = array(
			'INSERT'	=> 'to_id, from_id, ip, subject, message, hide_smilies, sent, first_message, reply_to',
			'INTO'		=> 'messages',
			'VALUES'	=> '\''.$to_id.'\', '.$forum_user['id'].', \''.get_remote_address().'\', \''.$forum_db->escape($subject).'\', \''.$forum_db->escape($message).'\', '.$hide_smilies.', '.$now.', '.$first_message.', '.$reply_to
		);
...
if ($id && substr($cur_posting['subject'],0,3) != 'Re:')
	$cur_posting['subject'] = 'Re: '.$cur_posting['subject'];

// Setup form
$forum_page['set_count'] = $forum_page['fld_count'] = 0;

if ($to)
	$forum_page['form_action'] = forum_link($forum_url['pm_send_to'], $to);
else if ($id)
	$forum_page['form_action'] = forum_link($forum_url['pm_reply'], $id);
else
	...
	<form id="afocus" class="frm-newform" method="post" accept-charset="utf-8" action="<?php echo $forum_page['form_action'] ?>"<?php if (!empty($forum_page['form_attributes'])) echo ' '.implode(' ', $forum_page['form_attributes']) ?>>
		...
		<fieldset class="frm-set set<?php echo ++$forum_page['set_count'] ?>">
			...
			<div class="frm-text required">
				...
				<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="req_to" value="<?php if (isset($_POST['req_to'])) echo forum_htmlencode($send_to); else if ($to || $id) echo $cur_posting['username']; ?>" size="35" maxlength="25" /></span>
			</div>
		</fieldset>
		<fieldset class="frm-set set<?php echo ++$forum_page['set_count'] ?>">
			...
			<div class="frm-text required longtext">
				...
				<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="text" name="req_subject" value="<?php if (isset($_POST['req_subject'])) echo forum_htmlencode($subject); else if ($id) echo forum_htmlencode($cur_posting['subject']); ?>" size="80" maxlength="70" /></span>
			</div>
