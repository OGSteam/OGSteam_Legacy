<?php
	$GMTsign = $db->first_result( "SELECT value
								   FROM " . DB_CONFIG_TABLE ."
								   WHERE name = 'gmtsign'" );
	
	if ( $GMTsign !== '' ) # Timezone
	{
		$GMT = intval($db->first_result( "SELECT value
										  FROM " . DB_CONFIG_TABLE ."
										  WHERE name = 'gmt'" ));
	...
	$DST = intval($db->first_result( "SELECT value
									  FROM " . DB_CONFIG_TABLE . "
									  WHERE name = 'dst'" )); # Daylight Saving Time {0 = false | 1 = true}

	$TD = $GMT + (3600 * $DST); # Time Difference

	$players_n = $db->first_result( "SELECT COUNT(nick)
									 FROM " . DB_PLAYERS_TABLE );
	$allys_n   = $db->first_result( "SELECT COUNT(DISTINCT ally)
									 FROM " . DB_PLAYERS_TABLE );
	$planets_n = $db->first_result( "SELECT COUNT(name)
									 FROM " . DB_PLANETS_TABLE );
	$last_upd  = $db->first_result( "SELECT MAX(date)
									 FROM " . DB_STATS_TABLE );
