...
function galaxy_show() {
	...
	$request = "select row, name, ally, player, moon, phalanx, gate, last_update_moon, status, ope, last_update, user_name";
	...
	while (list($row, $planet, $ally, $player, $moon, $phalanx, $gate, $last_update_moon, $status, $ope, $timestamp, $poster) = $db->sql_fetch_row($result)) {
		$report_spy = 0;
		//DIE("test2");
		...
		if (!in_array($ally, $ally_protection) || $ally == "" || $user_auth["server_show_positionhided"] == 1 || $user_data["user_admin"] == 1 || $user_data["user_coadmin"] == 1) {
			...
			$population[$row] = array("ally" => $ally, "player" => $player, "moon" => $moon, "phalanx" => $phalanx, "gate" => $gate, "last_update_moon" => $last_update_moon, "planet" => $planet, "report_spy" => $report_spy, "status" => $status, "ope" => $ope, "timestamp" => $timestamp, "poster" => $poster, "hided" => $hided, "allied" => $friend, "report_rc" => $report_rc);
		}
		elseif (in_array($ally, $ally_protection)) {
			$population[$row] = array("ally" => "", "player" => "", "moon" => "", "phalanx" => "", "gate" => "", "last_update_moon" => "", "planet" => "", "report_spy" => "", "status" => "", "ope" => "", "timestamp" => $timestamp, "poster" => $poster, "hided" => "", "allied" => "", "report_rc" => $report_rc);
		}
	}
...
function galaxy_system($lines) {
	...
	if ($OK) {
		...
		for ($i=$first_line+1 ; $i<sizeof($lines) ; $i++) {
			...
			$lines[$i] = str_replace('Envoyer un message Espionner  Ajouter aux amis','/',$lines[$i]);
			$lines[$i] = str_replace('  ','//',$lines[$i]);
		}
		...
		if ($optionconfort){
			...
			for ($i=0 ; $i<sizeof($newlines) ; $i++) {
				if (preg_match("#^[1-9][0-5]?/+[^/]+/*#", trim($newlines[$i]), $arr)) {
					$continue=true;
					$b=1;
					while($continue==true){
						if (empty($newlines[$i+$b])) $continue=false;
						$newlines[$i+$b] = preg_replace('#Activité détectée il y a ([1-6]?[0-9]min)#', ' ($1)', $newlines[$i+$b]);
						if (!preg_match('#^[1-9][0-5]?#', trim($newlines[$i+$b])) AND !preg_match('#\(/d planètes colonisées\)//Légende#', trim($newlines[$i+$b]))) $newlines[$i]=trim($newlines[$i]).$newlines[$i+$b];
						else $continue=false;
						$b++;
					}
		...
		for ($i=$first_line+1 ; $i<sizeof($lines) ; $i++) {
			...
			$regs = array();
			$result = array();
			if (preg_match ("#([1-9][0-5]?)/+([^/]*)/+([^/]*)/+([^/]*)/+#", $line, $line_ar)){
				$planet[$i-$first_line]["planet"]=$line_ar[2];
				$planet[$i-$first_line]["player"]=$line_ar[3];
				$planet[$i-$first_line]["ally"]=$line_ar[4];
				if (preg_match ("#(.*)\([1-6]?[0-9]min\)?#", $planet[$i-$first_line]["planet"], $line_ar)){
					$planet[$i-$first_line]["planet"]=$line_ar[1];
				}
				if (preg_match ("#(.*) Opé.?#", $planet[$i-$first_line]["player"], $line_ar)){
					$planet[$i-$first_line]["player"]=$line_ar[1];
					$planet[$i-$first_line]["ope"]=True;
				}
				if (preg_match ("#(.*)\( (.{0,8}) \)#", $planet[$i-$first_line]["player"], $line_ar)){
					$planet[$i-$first_line]["player"]=$line_ar[1];
		...
		for ($i=1 ; $i<=15 ; $i++) {
			if ($galaxy != "" && $system != "") {
				...
				if(!galaxy_add_system($galaxy, $system, $planet[$i]["row"], $planet[$i]["moon"], $planet[$i]["planet"], $planet[$i]["ally"], $planet[$i]["player"], $planet[$i]["status"], $planet[$i]["ope"], $now)) {
					return false;
				}
			}
		}
...
*/
function galaxy_add_system ($galaxy, $system, $row, $moon, $name, $ally, $player, $status, $ope, $timestamp, $mode_ogs = false) {
	global $db, $user_data;
	...
	if($last_update < $timestamp) {
		...
		//Mise à jour
		$request = "update ".TABLE_UNIVERSE." set ally = '".mysql_real_escape_string($ally)."', player = '".mysql_real_escape_string($player)."', moon = '".$moon."', name = '".mysql_real_escape_string($name)."', status = '".$status."', ope = '".$ope."', last_update = '".$timestamp."', last_update_user_id = ".$user_data["user_id"];
...
function galaxy_show_ranking_ally() {
	...
	$maxpoints = $db->sql_fetch_row($result);
	$maxrank[1] = $maxpoints[0];
	$request = "SELECT rank FROM `" .TABLE_RANK_ALLY_DEFENSES. "` order by `rank` desc limit 0,1";
	$result = $db->sql_query($request);
	$maxdenfense = $db->sql_fetch_row($result);
	$maxrank[3] = $maxdenfense[0];
	$request = "SELECT rank FROM `" .TABLE_RANK_ALLY_MINES. "` order by `rank` desc limit 0,1";
	$result = $db->sql_query($request);
	$maxmines= $db->sql_fetch_row($result);
	$maxrank[2] = $maxmines[0];
	$maxrank = max($maxrank);
	...
	switch ($pub_order_by) {
		...
		$table[] = array("tablename" => TABLE_RANK_ALLY_DEFENSES, "arrayname" => "defenses");
		$table[] = array("tablename" => TABLE_RANK_ALLY_MINES, "arrayname" => "mines");
		break;
		case "fleet":
		$table[] = array("tablename" => TABLE_RANK_ALLY_FLEET, "arrayname" => "fleet");
		$table[] = array("tablename" => TABLE_RANK_ALLY_POINTS, "arrayname" => "general");
...
function galaxy_show_ranking_unique_ally($ally, $last = false) {
	...
	$request = "select datadate, rank, points, number_member, points_per_member";
	$request .= " from ".TABLE_RANK_ALLY_MINES;
	$request .= " where ally = '".mysql_real_escape_string($ally)."'";
	$request .= " order by datadate desc";
	$result = $db->sql_query($request);
	while (list($datadate, $rank, $points, $number_member, $points_per_member) = $db->sql_fetch_row($result)) {
		$ranking[$datadate]["mines"] = array("rank" => $rank, "points" => $points, "points_per_member" => $points_per_member);
		$ranking[$datadate]["number_member"] = $number_member;
		if ($last) break;
	}
	
	$request = "select datadate, rank, points, number_member, points_per_member";
	$request .= " from ".TABLE_RANK_ALLY_DEFENSES;
	$request .= " where ally = '".mysql_real_escape_string($ally)."'";
	$request .= " order by datadate desc";
	$result = $db->sql_query($request);
	while (list($datadate, $rank, $points, $number_member, $points_per_member) = $db->sql_fetch_row($result)) {
		$ranking[$datadate]["defenses"] = array("rank" => $rank, "points" => $points, "points_per_member" => $points_per_member);
		$ranking[$datadate]["number_member"] = $number_member;
		if ($last) break;
	}
...
function galaxy_getranking($lines, $datatype) {
	...
	//Détection d type (player ou ally) Du classemant.
	if (preg_match("#general#", $datatype) or preg_match("#fleet#", $datatype) or preg_match("#research#", $datatype) or 
		preg_match("#mines#", $datatype) or preg_match("#defenses#", $datatype)) {
		for ($i=0 ; $i<sizeof($lines) ; $i++) {
			if (preg_match("#Alliance\(TAG\)#", $lines[$i])) {
				$datatype=$datatype."_ally";
				Break;
			}
			elseif($i==sizeof($lines)-1){
				$datatype=$datatype."_player";
			}
		}
	}
	switch ($datatype) {
	...
	if (preg_match("#player#", $datatype)) {
		...
		for ($i=0 ; $i<sizeof($lines) ; $i++) {
			...
			$line = str_replace('Envoyer un message  Ajouter aux amis','//',$line);
			$line = str_replace('  ','//',$line);
			$line = str_replace('/ ','//',$line);
			$line = str_replace(' /','//',$line);
			$line = trim($line);
		...
		for ($i=0 ; $i<sizeof($lines) ; $i++) {
			...
			$line = str_replace('/ ','//',$line);
			$line = str_replace(' /','//',$line);
			if (!$OK && preg_match("#Pos\.//Prog\.//Alliance\(TAG\)//Nb\. membres///Points////[a-zA-Z]*/Membre#", $line)) {
				$OK = true;
				$position = 0;
				continue;
			}
