...
$pub_action = "Xtense";
//if ($_SERVER['HTTP_USER_AGENT']!='OGSClient') exit('Vous devez installer la barre Xtense pour vous connecter via ce plugin Firefox<br><a href="http://ogsteam.fr/ogspyxtense/ogspyxtense109.xpi">Xtense Toolbar</a>');
...
if (mod_get_option("xtense_debug") == 1) define("XTENSE_PLUGIN_DEBUG", true);
...
if(isset($server_config)) $etat = "terminée\n";
else $etat = "echouée\n";
...
if (mod_get_option("xtense_maintenance") == 1) define("MAINTENANCE_SCRIPT", true); // en cas de maintenance

//Maintenant on s'occupe de la configuration du plugin
if (mod_get_option("xtense_logogssqlfailure")) $log_sql_errors = mod_get_option("xtense_logogssqlfailure");
else $log_sql_error=true;

if (mod_get_option("xtense_logogsplayerstats")) $log_ogsplayerstats = mod_get_option("xtense_logogsplayerstats");
else $log_ogsplayerstats=true;

if (mod_get_option("xtense_logogsallystats")) $log_ogsallystats = mod_get_option("xtense_logogsallystats");
else $log_ogsallystats=true;

if (mod_get_option("xtense_logogslogon")) $log_ogslogon = mod_get_option("xtense_logogslogon");
else $log_ogslogon=true;

if (mod_get_option("xtense_logogsspyadd")) $log_ogsspyadd = mod_get_option("xtense_logogsspyadd");
else $log_ogsspyadd=true;

if (mod_get_option("xtense_logogsgalview")) $log_ogsgalview = mod_get_option("xtense_logogsgalview");
else $log_ogsgalview=true;

if (mod_get_option("xtense_logogsallyhistory")) $log_ogsallyhistory = mod_get_option("xtense_logogsallyhistory");
else $log_ogsallyhistory=true;
   
if (mod_get_option("xtense_logogsuserbuildings")) $log_logogsuserbuildings = mod_get_option("xtense_logogsuserbuildings");
else $log_logogsuserbuildings=true;
 
if (mod_get_option("xtense_logogsusertechnos")) $log_logogsusertechnos = mod_get_option("xtense_logogsusertechnos");
else $log_logogsusertechnos=true;

if (mod_get_option("xtense_logogsuserdefence")) $log_logogsuserdefence = mod_get_option("xtense_logogsuserdefence");
else $log_logogsuserdefence=true;

if (mod_get_option("xtense_forcestricnameuniv")) $log_forcestricnameuniv = mod_get_option("xtense_forcestricnameuniv");
else $log_forcestricnameuniv=true;

if (mod_get_option("xtense_logunallowedconnattempt")) $log_logunallowedconnattempt = mod_get_option("xtense_logunallowedconnattempt");
else $log_logunallowedconnattempt=true;

if (mod_get_option("xtense_ogsplugin_nameuniv")) $log_ogsplugin_nameuniv = mod_get_option("xtense_ogsplugin_nameuniv");
else $log_ogsplugin_nameuniv='';

if (mod_get_option("xtense_handlegalaxyviews")) $plug_handlegalaxyviews = mod_get_option("xtense_handlegalaxyviews");
else $plug_handlegalaxyviews=true;
// if (!$plug_handlegalaxyviews) SendHttpStatusCode("780");      

if (mod_get_option("xtense_handleplayerstats")) $plug_handleplayerstats = mod_get_option("xtense_handleplayerstats");
else $plug_handleplayerstats=true;
// if (!$plug_handleplayerstats) SendHttpStatusCode("781");

if (mod_get_option("xtense_handleallystats")) $plug_handleallystats = mod_get_option("xtense_handleallystats");
else $plug_handleallystats=true;
// if (!$plug_handleallystats) SendHttpStatusCode("782");  

if (mod_get_option("xtense_handleespioreports")) $plug_handleespioreports = mod_get_option("xtense_handleespioreports");
...
if (($db->sql_affectedrows() == 0)  || $result==0  ) 
{
	...
	if ($is_ogsplugin == true) 
	{
        ...
        if (!mod_get_option("xtense_logogslogon") || mod_get_option("xtense_logogslogon")=='1')  log_plugin_('login_ogsplugin_new');
	}
}
else 
{ 
	...
	if (list($session_id, $session_user_id, $session_start, $session_expire, $session_ip, $session_ogs) = $db->sql_fetch_row($result)) 
	{
		...
		{ 
			...
			if (!mod_get_option("xtense_logogslogon") || mod_get_option("xtense_logogslogon")=='1')  log_plugin_('login_ogsplugin_upd');
		} 
		else 
		{ 
...
elseif (isset($pub_typ) && $pub_typ=="galaxy" && isset($pub_content) && !empty($pub_content) && isset($_POST['galaxy']) && is_numeric($_POST['galaxy'])&& isset($_POST['system']) && is_numeric($_POST['system'])) 
{
	...
	foreach($galaxies_array as $curr_gal_buf) 
	{
		...
		if (preg_match("#.*?Système solaire (\d+?):(\d+?)#", $solarstring, $solarstring_array)) 
		{
		...
		if (!mod_get_option("xtense_logogsgalview") || mod_get_option("xtense_logogsgalview")=='1')
		{
...
elseif (isset($pub_typ) && $pub_typ=="reports" && isset($pub_content) && !empty($pub_content))
{
	...
	if (!mod_get_option("xtense_logogsspyadd") || mod_get_option("xtense_logogsspyadd")=='1')  log_plugin_("load_spy_OGS",$num_spyadded);
