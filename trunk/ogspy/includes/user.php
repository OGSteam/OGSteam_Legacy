...
function member_user_set() {
	...
	$db->sql_query("UPDATE ".TABLE_USER." SET `off_amiral` = '0' WHERE `user_id` = ".$user_id);
	}
	
	//compte ingenieur
	if ($user_data['off_ingenieur'] == "0" && $pub_off_ingenieur == 1){
	$db->sql_query("UPDATE ".TABLE_USER." SET `off_ingenieur` = '1' WHERE `user_id` = ".$user_id);
	}
	if ($user_data['off_ingenieur'] == 1 && (is_null($pub_off_ingenieur) || $pub_off_ingenieur != 1)){
	$db->sql_query("UPDATE ".TABLE_USER." SET `off_ingenieur` = '0' WHERE `user_id` = ".$user_id);
	}
	
	//compte geologue
	if ($user_data['off_geologue'] == "0" && $pub_off_geologue == 1){
	$db->sql_query("UPDATE ".TABLE_USER." SET `off_geologue` = '1' WHERE `user_id` = ".$user_id);
	}
	if ($user_data['off_geologue'] == 1 && (is_null($pub_off_geologue) || $pub_off_geologue != 1)){
	$db->sql_query("UPDATE ".TABLE_USER." SET `off_geologue` = '0' WHERE `user_id` = ".$user_id);
	}
...
function user_set_building($data, $planet_id, $planet_name, $fields, $coordinates, $temperature_min, $temperature_max, $satellite) {
	...
	$link_building = array($lang_building["M"] => "M", $lang_building["C"] => "C", $lang_building["D"] => "D",
	$lang_building["CES"] => "CES", $lang_building["CEF"] => "CEF", $lang_building["DdR"] => "DdR",
	$lang_building["UdR"] => "UdR", $lang_building["UdN"] => "UdN", $lang_building["CSp"] => "CSp",
	$lang_building["HM"] => "HM", $lang_building["HC"] => "HC", $lang_building["HD"] => "HD",
	$lang_building["Lab"] => "Lab", $lang_building["Ter"] => "Ter", $lang_building["Silo"] => "Silo",
	$lang_building["BaLu"] => "BaLu", $lang_building["Pha"] => "Pha", $lang_building["PoSa"] => "PoSa");
	...
	foreach ($lines as $line) {
		...
		if (ereg("^(.*) \(Niveau ([[:digit:]]{1,3}).*\)$", $line, $arr)) {
			...
			if (isset($link_building[$building])) {
				...
				$res = $db->sql_query ( 'SELECT planet_name FROM ' . TABLE_USER_BUILDING . ' WHERE planet_id = ' . $planet_id );
...
function user_move_empire() {
	...
	if (isset($pub_left)) {
		if ($pub_planet_id == 101) redirection("index.php?action=home&subaction=empire");
		$new_position = $pub_planet_id-1;
	}
	elseif (isset($pub_right)) {
		if ($pub_planet_id == (100+$nb_planete)) redirection("index.php?action=home&subaction=empire");
		$new_position = $pub_planet_id+1;
	}
...
function user_getfavorites_spy() {
	...
	$request = "select ".TABLE_PARSEDSPY.".id_spy, coordinates, dateRE, sender_id, ".TABLE_UNIVERSE.".moon, ".TABLE_UNIVERSE.".ally, ".TABLE_UNIVERSE.".player, ".TABLE_UNIVERSE.".status";
	$request .= " from ".TABLE_PARSEDSPY.", ".TABLE_USER_SPY.", ".TABLE_UNIVERSE;
	$request .= " where user_id = ".$user_data["user_id"]." and CONCAT(".TABLE_UNIVERSE.".galaxy,':',".TABLE_UNIVERSE.".system,':',".TABLE_UNIVERSE.".row)=coordinates and ".TABLE_USER_SPY.".spy_id=".TABLE_PARSEDSPY.".id_spy";
	...
	while (list($spy_id, $coordinates, $datadate, $sender_id, $moon, $ally, $player, $status) = $db->sql_fetch_row($result)) {
		...
		$favorite[$spy_id] = array("spy_id" => $spy_id, "spy_galaxy" => substr( $coordinates, 0, strpos( $coordinates, ':' ) ), "spy_system" => substr( $coordinates, strpos ( $coordinates, ':' ) + 1 , strrpos ( $coordinates, ':' ) - strpos ( $coordinates, ':' ) - 1 ), "spy_row" => substr( $coordinates, strrpos( $coordinates, ':' ) + 1 ), "player" => $player, "ally" => $ally, "moon" => $moon, "status" => $status, "datadate" => $datadate, "poster" => $user_name);
	}
...
function parseRC ( $rawRC )
{
  ...
  preg_match ( '/Un champ de d.bris contenant (\d*) unit.s de m.tal et (\d*) unit.s de cristal(.*)/', $rawRC, $reg );
  ...
  if ( preg_match ( '/L\'attaquant a gagn. la bataille/', $rawRC ) )
  {
    ...
    preg_match ( '/(\d*) unit.s de m.tal, (\d*) unit.s de cristal et (\d*) unit.s de deut.rium/', $rawRC, $reg );
...
function UNparseRC ( $id_RC )
{
  ...
  for ( $idx = 1; $idx <= $nb_rounds; $idx++ )
  {
    ...
    while ( list ( $player, $coordinates, $Armes, $Bouclier, $Protection, $PT, $GT, $CLE, $CLO, $CR, $VB, 
      $VC, $REC, $SE, $BMD, $DST, $EDLM, $TRA ) = $db->sql_fetch_row ( $result_attack ) )
    {
      ...
      $template .= 'Attaquant ' . $player . ' ([' . $coordinates . '])' . "\n" . 'Armes: ' . $Armes . 
        '% Bouclier: ' . $Bouclier . '% Coque: ' . $Protection . '%' . "\n";
      $ship_type = 'Type';
      $ship_nombre = 'Nombre';
      $ship_armes = 'Armes';
      ...
      $VC, $REC, $SE, $BMD, $DST, $EDLM, $TRA, $LM, $LLE, $LLO, $CG, $AI, $LP, $PB, $GB ) = $db->sql_fetch_row ( $result_defense ) )
    {
      ...
      $ship = 0;
      $vivant_def = false;
      $template .= 'Défenseur ' . $player . ' ([' . $coordinates . '])' . "\n" . 'Armes: ' . $Armes . '% Bouclier: ' . $Bouclier . '% Coque: ' . $Protection . '%' . "\n";
      $ship_type = 'Type';
      $ship_nombre = 'Nombre';
      $ship_armes = 'Armes';
      $ship_bouclier = 'Bouclier';
  ...
  switch ( $victoire )
  {
    case 'N':
      ...
      break;
    case 'A':
      $template .= 'L\'attaquant a gagné la bataille !' . "\n" . 'Il emporte' . "\n" . $gain_M . ' unités de métal, ' . $gain_C . ' unités de cristal et ' . $gain_D . ' unités de deutérium' . "\n\n\n";
      break;
    case 'D':
  ...
  $template .= 'Un champ de débris contenant ' . $debris_M . ' unités de métal et ' . $debris_C . ' unités de cristal se forme dans l\'orbite de cette planète.' . "\n";
