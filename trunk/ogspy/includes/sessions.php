...
function session_begin($user_ip) {
	...
	if (strstr($_SERVER['HTTP_USER_AGENT'],"OGSClient") === false) {
		...
		$db->sql_query($request, true, false) or die(L_("incsession_init"));
	}
	else {
...
function session_set_user_data($cookie_id) {
	...
	elseif (preg_match("#^3.05(beta)?(-stable)?$#", $server_config["version"])) {
		$request = "select user_id, user_name, user_admin, user_coadmin, user_galaxy, user_system, user_skin, session_lastvisit, user_stat_name, ";
		$request .= "management_user, management_ranking, disable_ip_check, off_amiral, off_ingenieur, off_geologue, off_technocrate";
	}
	else {
		$request = "select user_id, user_name, user_admin, user_coadmin, user_galaxy, user_system, user_row, user_skin, session_lastvisit, user_stat_name, ";
		$request .= "management_user, management_ranking, disable_ip_check, off_amiral, off_ingenieur, off_geologue, off_technocrate, user_language";
	}//*/
