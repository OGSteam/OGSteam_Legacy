...
if((! reg.test(url)) || (new RegExp(/xtense=Options/)).test(url)){
	...
	if(url.indexOf('xtense=Options',0) >= 0){
		if(isChrome){	
			...
			options += '<thead><tr><th class="Xtense_th" colspan="2" style="font-size: 12px; text-align:center; font-weight: bold; color: #539fc8; line-height: 30px; height: 30px;"><img src="http://svn.ogsteam.fr/trunk/xtense_GreaseMonkey/xtense.png" alt="Options Xtense" height="150%"/></th></tr></thead>';
			options+= '<tbody>';
			// Serveur 1
			options+= '<tr>';
			options+= '<td class="champ"><label class="styled textBeefy">URL Univers</label></td>';
			options+= '<td class="value"><input class="speed" id="server1.url.univers" value="'+GM_getValue('server1.url.univers','http://uniXXX.ogame.fr')+'" size="35" alt="24" type="text"/></td>';
			options+= '</tr>';
			options+= '<tr>';
		...
		// Firefox ?
			...
			"<img src=\"http://svn.ogsteam.fr/trunk/xtense_GreaseMonkey/OGSteam.gif\" alt\"OGSteam\"/>&#160;&#160;&#160;" +
			"<img src=\"http://svn.ogsteam.fr/trunk/xtense_GreaseMonkey/xtense.png\" alt\"Xtense\"/>" +			
			"<br/><br/>" +
			"Une extension Firefox de ce script existe." +
			"<br/><br/>" +
			"Veuillez la télécharger afin de profiter pleinement de cet outil." +
			"<br/><br/><br/>" +
			"Ce module complémentaire est disponible à l'<a href=\"http://update.ogsteam.fr/xtense/download.php\">adresse suivante.</a>" +
			"<br/><br/>" +
			"</div></div>";
