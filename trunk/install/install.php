<?php
/**
 * Fichier d'installation d'SpacSpy : ROOT/install/install.php 
 * @package SpacSpy
 * @subpackage install
 * @author Kyser
...
<html>
<head>
<title>Installation SpacSpy</title>
...
if ($alerte) {
	...
	echo "<tr><td colspan='2'>Pour le bon fonctionnement et une installation complète de SpacSpy,<br />vous devez";
	echo " autoriser ces dossiers en écriture";
	echo $error;
	echo "<tr><th colspan='2'><font color='red'>Erreur optionelle :</font></th><tr/>";
	echo "<tr><td colspan='2'>Ces dossiers servent à l'installation et à la mise à jour des modules SpacSpy.<br>";
...
function error_sql($message) {
	echo "<h3 align='center'><font color='red'>Erreur durant la procédure d'installation du serveur SpacSpy</font></h3>";
...
* @var string $sgbd_tableprefix Préfixe à utiliser pour les tables SpacSpy
* @var string $admin_username Nom de l'Administrateur SpacSpy
* @var string $admin_password Mot de passe Administrateur SpacSpy
* @var string $admin_password2 Confirmation du Mot de passe Administrateur SpacSpy
* @var int $num_of_galaxies Nombre de galaxies dans l'univers spacecon de cet SpacSpy
* @var int $num_of_systems Nombre de systèmes dans l'univers spacecon de cet SpacSpy
*/
function installation_db($sgbd_server, $sgbd_dbname, $sgbd_username, $sgbd_password, $sgbd_tableprefix, $admin_username, $admin_password, $admin_password2, $num_of_galaxies, $num_of_systems) {
	...
	$sql_query = @fread(@fopen("schemas/spacspy_structure.sql", 'r'), @filesize("schemas/spacspy_structure.sql")) or die("<h1>Le script sql d'installation est introuvable</h1>");

	$sql_query = preg_replace("#spacspy_#", $sgbd_tableprefix, $sql_query);
...
function generate_id($sgbd_server, $sgbd_dbname, $sgbd_username, $sgbd_password, $sgbd_tableprefix, $sgbd_num_of_galaxies, $sgbd_num_of_systems) {
	...
	$id_php[] = '***************************************************************************/';
	$id_php[] = '';
	$id_php[] = 'if (!defined("IN_SPACSPY")) die("Hacking attempt");';
	$id_php[] = '';
	...
	$id_php[] = '';
	$id_php[] = 'define("SpacSpy_INSTALLED", TRUE);';
	$id_php[] = '?>';
	if (!write_file("../parameters/id.php", "w", $id_php)) {
		die("Echec installation, impossible de générer le fichier 'parameters/id.php'");
	}

	echo "<h3 align='center'><font color='yellow'>Installation du serveur SpacSpy 0.1b effectuée avec succès</font></h3>";
	echo "<center>";
...
<tr>
	<td align="center">
		<table width="800">
		<tr>
			<td colspan="2" align="center"><font size="3"><b>Bienvenue à l'installation d'SpacSpy version 0.1b</b></font></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<th><input name="sgbd_tableprefix" type="text" value="<?php echo isset($pub_sgbd_tableprefix) ? $pub_sgbd_tableprefix : "SpacSpy_";?>"></th>
		</tr>
		<tr>
		...
		<tr><td colspan="2">&nbsp;</td></tr>
		<tr>
			<td colspan="2" align="center"><a target="_blank" href="http://ogsteam.fr/SpacSpynstall/"><i><font color="orange">Besoin d'assistance ?</font></i></a></td>
		</tr>
		</table>
	</td>
</tr>
