...
for ($i=0 ; $i<12 ; $i++) {
	...
	if ($show == $show_year."~".$show_month."~".$show_day) {
		...
		if (log_check_exist(date("ym", $date))) {
			echo "<input type='image' src='images/save.png' onclick=\"window.location = 'index.php?action=extractor&date=".date("ym", $date)."'\" title='Télécharger les logs de ".strftime("%B %Y", $date)."'></font>";
		}
		else echo "&nbsp;";
		echo "</th>"."\n";
	}
...
for ($i=1 ; $i<=$max_day ; $i++) {
	...
	if ($show == $show_year."~".$show_month."~".$show_day) {
		...
		if (log_check_exist($show_year.$show_month.$day)) {
			echo "<input type='image' src='images/save.png' onclick=\"window.location = 'index.php?action=extractor&date=".$show_year.$show_month.$day."'\" title='Télécharger les logs de ".strftime("%d %B %Y", $date)."'>";
		}
		else echo "&nbsp;";
		echo "</th>"."\n";
	}
...
?>
<tr>
	<td><font color="Red"><i>Si vous souhaitez visualiser les fichiers transactionnels, télechargez les logs</i></font></td>
</tr>
</table>
