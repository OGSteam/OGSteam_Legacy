...
if (sizeof($usergroup_member) > 0) {
	...
	foreach ($usergroup_member as $user) {
		...
		echo "\t"."<form method='POST' action='index.php?action=usergroup_delmember&user_id=".$user["user_id"]."&group_id=".$group_id."' onsubmit=\"return confirm('Etes-vous sûr de vouloir supprimer ".$user["username"]." du groupe ?');\">"."\n";
		echo "\t"."<th width='175'>".$user["username"]."</th><th width='25'><input type='image' src='images/userdrop.png' title='Supprimer ".$user["username"]." du groupe'></th>";
		echo "\t"."</form>"."\n";
		$index++;
	}
