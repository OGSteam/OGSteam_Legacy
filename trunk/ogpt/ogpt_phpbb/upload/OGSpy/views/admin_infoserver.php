...
if ($fsock = @fsockopen($url_server, 80, $errno, $errstr, 3)) {
	...
	if (preg_match("#¤(\d+\.\d+[_a-z]*)¤#m", $version_info, $version_info)) {
		...
		if ($head_revision == $latest_head_revision && $minor_revision == $latest_minor_revision && $extension_revision == $latest_extension_revision) {
			...
			$version_info .= "<br />Rendez vous sur le  <a href='http://ogs.servebbs.net' target='_blank'>forum</a> dédié au support d'OGSpy pour récupérer la dernière version : <font color='red'>".$latest_version."</b>";
		}
	}
	else {
		...
		$version_info .= "<br />Consulter le <a href='http://ogs.servebbs.net/forums/index.php' target='_blank'>forum</a> dédié au support d'OGSpy pour en connaître la raison.</b>";
	}
}
else {
	...
	$version_info .= "<br />Il vous faudra consulter régulièrement le <a href='http://ogs.servebbs.net/forums/index.php' target='_blank'>forum</a> dédié au support d'OGSpy pour prendre connaissance des nouvelles versions.</b>";
}
?>
