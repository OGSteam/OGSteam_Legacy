...
if (!$forum_user['is_guest'])
{
	...
	($hook = get_hook('pr_qr_get_user_count')) ? eval($hook) : null;
	...
	($hook = get_hook('pr_qr_get_post_stats')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
	...
	($hook = get_hook('pr_pre_users_online')) ? eval($hook) : null;
	...
	if ($forum_config['o_users_online'] == '1')
	{
		...
		($hook = get_hook('pr_qr_get_users_online')) ? eval($hook) : null;
...
?>
			...
			<br />
			<form method="post" action="<?php echo $base_url ?>/login.php?action=in">
				<div class="hidden">
