...
if (($system_down+$totalsystem+1) <= $server_config["nb_system"]) {
...
echo "<td class='c'>".$link_left1.$LANG["galaxysector_Previous"].$link_left2."</td>";
echo "<td class='c' colspan='3'>".$LANG["galaxysector_Navigation"]."<br />".$galaxy.":".$system_down." - ".$galaxy.":".$system_up."</td>";
echo "<td class='c'>".$link_right1.$LANG["galaxysector_Next"].$link_right2."</td>"."\n";
echo "</tr>"."\n";
for ($lines=0 ; $lines<ceil($totalsystem/5) ; $lines++) {
	...
	for ($cols=$system ; $cols<$system+5 ; $cols++) {
		...
		echo "</tr>";
		for ($row=1 ; $row<=$server_config["nb_galaxy"] ; $row++) {
			$head = $row;
			$box = "&nbsp";
