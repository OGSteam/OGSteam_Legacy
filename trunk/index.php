<?php
/**
 * Fichier principal d'SpacSpy : index.php appellé lors de tout accés
 * @package SpacSpy
 * @subpackage main
 * @author Kyser
...
/**
 * Repère de début de traitement par SpacSpy
 * @name $php_start
 */
$php_start = benchmark();
$sql_timing = 0;
...
if (strstr($_SERVER['HTTP_USER_AGENT'], "spacsClient") === false) {
	if (MODE_DEBUG) {
	...
	if (is_dir("install") && $pub_action != "message") {
		...
		if ($server_config["version"] < $install_version) {
	...
	if ($pub_action <> '') {
        $query = "SELECT root, link FROM ".TABLE_MOD." WHERE active = '1' AND action = '".mysql_real_escape_string($pub_action)."'";
