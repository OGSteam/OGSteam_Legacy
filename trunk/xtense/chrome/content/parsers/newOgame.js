...
var XnewOgame = {
	//on d√©clare les varibles generales
	locales: {},
	doc : null,
	url : null,
	Tab : null,
	...
	getPage: function(url) {
		try {
			this.setParams(url);
			// Nettoyage de la chaine, on ne prend que ce qu'il y a aprÔøΩs index.php?page(>>)
		...
		try {
			...
			if(rows.snapshotLength > 0){
				...
				for (var i = 1; i < rows.snapshotLength; i++) {
					...
					if (i == 1) {
						offset = Math.floor(n/100)*100+1;//parce que le nouveau classement ne commence pas toujours pile ÔøΩ la centaine et OGSpy toujours ÔøΩ 101,201...
					}
					if (type[0] == 'player') {
			...
			if(rounds.snapshotLength > 0){
				for(var div=0;div<rounds.snapshotLength;div++){
					...
					if(div == 0){
						...
						if(m){
							// Calcul heure d'ÈtÈ => offset = -120 & heure d'hiver  => offset = -60
	...
	getElementInSpyReport : function (RE,elem) {
		var num = -1;
		var reg = new RegExp(elem+'\\D+(\\d[\\d.]*)');//rÔøΩcupÔøΩre le nombre le plus proche aprÔøΩs le texte
