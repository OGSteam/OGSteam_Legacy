...
// Create page object
$mod_version_edit = new cmod_version_edit();
$Page =& $mod_version_edit;

// Page init
$mod_version_edit->Page_Init();

// Page main
$mod_version_edit->Page_Main();
?>
...
// Create page object
var mod_version_edit = new ew_Page("mod_version_edit");

// page properties
mod_version_edit.PageID = "edit"; // page ID
mod_version_edit.FormID = "fmod_versionedit"; // form ID
var EW_PAGE_ID = mod_version_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
mod_version_edit.ValidateForm = function(fobj) {
...
mod_version_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
mod_version_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
mod_version_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mod_version_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $mod_version->TableCaption() ?><br><br>
<a href="<?php echo $mod_version->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$mod_version_edit->ShowMessage();
?>
<form name="fmod_versionedit" id="fmod_versionedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mod_version_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="mod_version">
...
<?php if ($mod_version->id_mod_version->Visible) { // id_mod_version ?>
	<tr<?php echo $mod_version->RowAttributes() ?>>
		...
		<td<?php echo $mod_version->id_mod_version->CellAttributes() ?>><span id="el_id_mod_version">
<div<?php echo $mod_version->id_mod_version->ViewAttributes() ?>><?php echo $mod_version->id_mod_version->EditValue ?></div><input type="hidden" name="x_id_mod_version" id="x_id_mod_version" value="<?php echo ew_HtmlEncode($mod_version->id_mod_version->CurrentValue) ?>">
</span><?php echo $mod_version->id_mod_version->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$mod_version_edit->Page_Terminate();
?>
<?php
...
class cmod_version_edit {
	...
	// Page object name
	var $PageObjName = 'mod_version_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$mod_version->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$mod_version->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$mod_version->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$mod_version->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($mod_version->id_mod_version->CurrentValue == "")
			$this->Page_Terminate("mod_versionlist.php"); // Invalid key, return to list
		...
		// Render the record
		$mod_version->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($mod_version->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// id_mod_version
			$mod_version->id_mod_version->EditCustomAttributes = "";
			$mod_version->id_mod_version->EditValue = $mod_version->id_mod_version->CurrentValue;
			$mod_version->id_mod_version->CssStyle = "";
			$mod_version->id_mod_version->CssClass = "";
			$mod_version->id_mod_version->ViewCustomAttributes = "";
