...
fctIsInSys = function(strColor) {
	...
	for (ii = 0; ii < arrSys.length; ii++) {
		if (arrSys[ii][0] == strColor) {
			strOut = arrSys[ii][1]; break;
		}
	}
	return strOut;
}
...
fctIsInName = function(strColor) {
	...
	for (ii = 0; ii < arrName.length; ii++) {
		if (arrName[ii][0] == strColor) {
			strOut = arrName[ii][1]; break;
		}
	}
	return strOut;
}
...
fctShow = function(objForm) {
	if (objForm) {
		...
		if (objForm.value + '' != '') {
			...
			if (strColorTxt == '') {
				strColorTxt = '' + fctIsInSys(strColor);
			}
...
objSB.Append('<td width="33%" align="center"><input type="button" value="Quitter" class="btnColor" onClick="fctHide();"></td>');
