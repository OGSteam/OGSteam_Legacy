...
if(class_exists("Callback")) {
	class cdr_Callback extends Callback {
		...
		public function cdr($system) {
			global $io;
			if (cdr($system)) return Io::SUCCESS;
...
function cdr($system) {
	...
	for ($i=0; $i<count($system['data']); $i++) {
		...
		if (isset($system['data'][$rows]['debris'])) { //après Xtense 2.2
			...
			$cristal = isset($system['data'][$rows]['debris']['cristal']) ? $system['data'][$rows]['debris']['cristal'] : $system['data'][$rows]['debris']['crystal'];
		...
		if ($total !== 0 && $total > 5000) {
			...
			$query = "INSERT INTO ".TABLE_CDR
				." (date, total, metal, cristal, gal, coord)"
