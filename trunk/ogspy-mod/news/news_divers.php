...
/*** Raccourci sur Commandes dans divers ***/
$naoc="?action=News&amp;subaction=Divers&amp;command";

/*** Présentation des news ***/
echo "<table width=90%>\n
		<tr>\n
			\t<td class=c colspan=3 align='center'>Informations diverses</td>\n
		</tr>\n
		<tr>\n
			\t<td class=c colspan=3 align='center'>
				<form action='?action=News&amp;subaction=Divers' method='POST'>Informations sur :
					<select name='mod'>";
/*** Selection de la rubrique ***/
$query = "SELECT DISTINCT `idcat`, `SousCat`
			FROM `".$table_prefix."news`, `".$table_prefix."news_cat`
			WHERE `idcat` in (select `id` from `".$table_prefix."news_cat` where `categorie`='Divers')
...
/*** Fin selection de rubrique ***/
	...
	switch($pub_command) {
		...
		case "post":
			...
			redirection("?action=News&amp;subaction=Divers&amp;mod=".$pub_mod);
			break;
...
if($droit['post']) {
	echo "<table width=90%>\n
		...
		$query = "SELECT `id`, `SousCat` FROM `".$table_prefix."news_cat` WHERE `categorie`='Divers' ORDER BY `SousCat`";
		$result=$db->sql_query($query);
		while ($row=$db->sql_fetch_assoc($result)){
			if ($pub_mod == $row["id"]) {
