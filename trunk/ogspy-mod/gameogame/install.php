...
if ($is_ok == true)
	{
		...
		$queries[] = 'CREATE TABLE '.$table_prefix.'game_recyclage (
			...
			//  `GT` int(11) NOT NULL default '-1',
			//  `CLE` int(11) NOT NULL default '-1',
			//  `CLO` in	t(11) NOT NULL default '-1',
			//  `CR` int(11) NOT NULL default '-1',
			//  `VB` int(11) NOT NULL default '-1',
			//  `VC` int(11) NOT NULL default '-1',
			//  `REC` int(11) NOT NULL default '-1',
			...
			if( mysql_num_rows( mysql_query("SHOW TABLES LIKE '".$table_prefix."xtense_callbacks"."'")))
				{
					...
					$db->sql_query($query);
					echo("<script> alert('La compatibilité du mod gameOgame avec le mod Xtense2 est installée !') </script>");
				}
			else
				{
				echo("<script> alert('Le mod Xtense 2 n\'est pas installé. \nLa compatibilité du mod gameOgame ne sera donc pas installée !\nPensez à installer Xtense 2 c'est pratique ;)') </script>");
				}
	}
else
	{
