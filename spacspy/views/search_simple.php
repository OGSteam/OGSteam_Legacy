...
<tr>
	...
	<td width="400" valign="top">
		...
		<input type="hidden" name="action" value="search">
		<tr>
			<td class="c" colspan="4">Recherche spéciale</td>
		</tr>
		<tr>
			<th colspan="2">
...
foreach ($search_result as $v) {
	...
	if ($v["ally"] == "") $ally = "&nbsp;";
		...
		while ($ranking = current($individual_ranking_ally)) {
			...
			$tooltip[$v["ally"]] .= "<tr><td class=\"c\">Mines</td><th>".$mines_rank."</th><th>".$mines_points."</th></tr>";
			$tooltip[$v["ally"]] .= "<tr><td class=\"c\">Défenses</td><th>".$defenses_rank."</th><th>".$defenses_points."</th></tr>";				
	...
	if ($v["player"] == "") $player = "&nbsp;";
		...
		if (!isset($tooltip[$v["player"]])) {
			...
			while ($ranking = current($individual_ranking_player)) {
				...
				$tooltip[$v["player"]] .= "<tr><td class=\"c\">Mines</td><th>".$mines_rank."</th><th>".$mines_points."</th></tr>";
				$tooltip[$v["player"]] .= "<tr><td class=\"c\">Défenses</td><th>".$defenses_rank."</th><th>".$defenses_rank."</th></tr>";			
				break;
			}
...
<tr>
	<td colspan="3" align="center">
		<table>
		<tr>
			<td class="c" colspan="12">Classement de <a><?php echo $string_search;?></a></td>
		</tr>
		<tr>
			<td class="c">Date</td>
			...
			<td class="c" colspan="2">Pts defenses</td>
...
while ($ranking = current($individual_ranking)) {
	...
	echo "\t\t\t"."<th width='70'>".$mines_points."</th>"."\n";
	echo "\t\t\t"."<th width='40'><font color='lime'><i>".$mines_rank."</i></font></th>"."\n";
	echo "\t\t\t"."<th width='70'>".$defenses_points."</th>"."\n";
