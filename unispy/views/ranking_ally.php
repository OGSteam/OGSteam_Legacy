...
list($order, $ranking, $ranking_available) = galaxy_show_ranking_ally();

$order_by = $pub_order_by;
$interval = $pub_interval;

$tooltip = "<table width=\"120\">";
$tooltip .= "<tr><td class=\"c\"><a href=\"index.php?action=ranking&subaction=ally&order_by=type\">".$LANG["rankingally_Orderglobal"]."</a></td></tr>";
$tooltip .= "<tr><td class=\"c\"><a href=\"index.php?action=ranking&subaction=ally&order_by=type&suborder=member\">".$LANG["rankingally_Orderbymember"]."</a></td></tr>";
$tooltip .= "</table>";
$tooltip = addslashes(htmlentities($tooltip));

$link_general = "<a href=\"index.php?action=ranking&subaction=ally&order_by=general\" onmouseover=\"this.T_WIDTH=120;return escape('".str_replace("type", "general", $tooltip)."')\">".$LANG["rankingally_Ptsgeneral"]."</a>";
$link_fleet = "<a href=\"index.php?action=ranking&subaction=ally&order_by=fleet\" onmouseover=\"this.T_WIDTH=120;return escape('".str_replace("type", "fleet", $tooltip)."')\">".$LANG["rankingally_Ptsfloat"]."</a>";
$link_research = "<a href=\"index.php?action=ranking&subaction=ally&order_by=research\" onmouseover=\"this.T_WIDTH=120;return escape('".str_replace("type", "research", $tooltip)."')\">".$LANG["rankingally_Ptsresearch"]."</a>";

switch ($order_by) {
	case "general": $link_general = str_replace("Pts Général", "<img src='images/asc.png'>&nbsp;".$LANG["rankingally_Ptsgeneral"]."&nbsp;<img src='images/asc.png'>", $link_general);break;
	case "fleet": $link_fleet = str_replace("Pts Flotte", "<img src='images/asc.png'>&nbsp;".$LANG["rankingally_Ptsfloat"]."&nbsp;<img src='images/asc.png'>", $link_fleet);break;
	case "research": $link_research = str_replace("Pts Recherche", "<img src='images/asc.png'>&nbsp;".$LANG["rankingally_Ptsresearch"]."&nbsp;<img src='images/asc.png'>", $link_research);break;
}
...
if ($user_data["user_admin"] == 1 || $user_data["user_coadmin"] == 1 || $user_data["management_ranking"] == 1) {?>
	<form method="POST" action="index.php" onsubmit="return confirm('<?php echo $LANG["rankingally_Areyousure"]; ?>');">
	<input type="hidden" name="action" value="drop_ranking">
	<input type="hidden" name="subaction" value="ally">
	<input type="hidden" name="datadate" value="<?php echo $datadate;?>">
	<td align="right"><input type="image" src="images/drop.png" title="<?php echo $LANG["rankingally_Deletefillingof"]; ?> <?php echo $date_selected;?>"></td>
	</form>
<?php }?>
</tr>
</table>
<table width="790">
<tr>
	<td class="c" width="30"><?php echo $LANG["rankingally_Place"]; ?></td>
...
while ($value = current($order)) {
	$ally = "<a href='index.php?action=search&type_search=ally&string_search=".urlencode($value)."&strict=on'>";
	$ally .= $value;
	$ally .= "</a>";
	...
	echo "\t"."<th>".$ally."</th>";
//	echo "\t"."<th>".$member."</th>";
	echo "\t"."<th width='90'>".$general_pts."</th>";
...
//	echo "\t"."<th width='40'><font color='yellow'><i>".$fleet_pts_per_member."</i></font></th>";
	...
	echo "\t"."<th width='90'>".$research_pts."</th>";
