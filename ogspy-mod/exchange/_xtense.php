...
class eXchange_Callback extends Callback {
        ...
        public function eXchange_xtense2_msg($msg){
			...
			$checksomme = 0;
			foreach ($msg as $mess)
				if(eXchange_analyse_moi_ce_message_perso($user_data['user_id'], $mess['coords'][0], $mess['coords'][1], $mess['coords'][2], addslashes($mess['from']), addslashes($mess['subject']), addslashes($mess['message']), $mess['time']))
					$checksomme++;
			if($checksomme>0)
		...
		public function eXchange_xtense2_ally_msg($ally_msg){
			...
			foreach ($ally_msg as $mess)
				if(eXchange_analyse_moi_ce_message_d_alliance($user_data['user_id'], addslashes($mess['from']), addslashes($mess['tag']), addslashes($mess['message']), $mess['time']))
					$checksomme++;
				if($checksomme>0)
