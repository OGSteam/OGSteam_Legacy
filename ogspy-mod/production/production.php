...
if (file_exists("mod/production/lang/lang_".$server_config['language'].".php")) 
	require_once("mod/production/lang/lang_".$server_config['language'].".php");
if (file_exists("mod/production/lang/lang_".$user_data['user_language'].".php")) 
...
if (isset($_POST['techno_energie'])) {
	...
	if (isset($_POST['ingenieur']) && $_POST['ingenieur'] == 'on') 
		$ingenieur = '1';
	if (isset($_POST['geologue']) && $_POST['geologue'] == 'on') 
		$geologue = '1';
...
for ($i=1;$i<=9;$i++) {
	if ($user_building[$i]['planet_name'] != '') {
		...
		echo "batimentsOGSpy[".$i."] = new Array('".
			...
			$user_building[$i]['CEF']."','".
			$user_building[$i]['Sat']."','".
			$user_building[$i]['temperature_min']."','".
...
function selection(sel) {
	if (sel == 0) 
		...
		sel = true;
	for (i=1;i<=9;i++) 
		document.getElementById('global' + i).checked = sel;
	verif_donnee ();
}
...
function verif_donnee(envoye) {
	...
	?>
	if ((isNaN(parseFloat(document.getElementById('techno_energie').value))) || parseFloat(document.getElementById('techno_energie').value) < 0 ) 
		document.getElementById('techno_energie').value = technologieNRJ;
...
function calcul() {
	...
	for (i=1;i<=9;i++) {
		if (batimentsOGSpy[i][15] == 1) {
			...
			deut_heure[i] = vitesse * Math.round(geologue * Math.round(production("D", donnee['D'][i], batimentsOGSpy[i][8], technologieNRJ) * (donnee['rap_D'][i] / 100) * ratio[i]) - Math.round(consumption("CEF", donnee['FR'][i]) * donnee['rap_FR'][i] / 100));
