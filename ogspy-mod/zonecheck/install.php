...
$query = 'CREATE TABLE IF NOT EXISTS `' . TABLE_MOD_ZONECHECK . '` (
  `galaxy` enum ( ';
for ( $idx_g = 1; $idx_g < $num_of_galaxies; $idx_g++)
  $query .= '\'' . $idx_g . '\', ';
$query .= '\'' . ( $idx_g + 1 ) . '\'';
...
if ( ! defined ( 'TABLE_MOD_CFG' ) )
{
  ...
  $db->sql_query ( $query );
  $query = 'INSERT IGNORE INTO ' . TABLE_MOD_ZONECHECK_CFG . ' VALUES ( 50, "#FFAAD4", "#00D4FF", "#DDBB66", "#7FFFFF", "#FF7F00", "#FFAA2A", "#FF0000", "#FF2A00", "#FFAA2A", "#FFFF2A", "#FFFFFF", "\inactif|I|Lune|Colonisation")';
  $db->sql_query ( $query );
}
else
{
...
$query = 'INSERT IGNORE INTO ' . TABLE_MOD . ' (id, title, menu, action, root, link, version, active) VALUES (\'\',\'Zone Check\',\'Zone Check\',\'ZoneCheck\',\'ZoneCheck\',\'index.php\',\'2.6\',\'1\')';
$db->sql_query($query);
?>
