...
$xtense_version="2.00b1";

function ally_list($rapport)
  {
  global $sql, $table_prefix, $user ;
  //var_dump($rapport);
  ...
  for ($i=0; $i < count($rapport['list']); $i++)
      {
      $query = "INSERT INTO `".TABLE_RANK_MEMBERS."` VALUES (".$datadate.",'".$rapport['list'][$i]['pseudo']."',".$rapport['list'][$i]['points'].",'".$alliance."',".$user['id'].")";
	  $sql->query($query);
      }
  return TRUE;
  }
