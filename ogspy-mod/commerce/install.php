...
define("TABLE_COMMERCE", $table_prefix."commerce_commerce");
...
//$query = "INSERT INTO ".TABLE_MOD." ( title, menu, action, root, link, version, active) VALUES ( 'Commerce', 'Gestion des Commerces', 'commerce', 'commerce', 'index.php', '".trim($version_txt[1])."', '1')";
//$db->sql_query($query);
...
if ($is_ok == true){
		...
		$query = "SELECT `id` FROM `".TABLE_MOD."` WHERE `action`='commerce' AND `active`='1' LIMIT 1";
		$result = $db->sql_query($query);
		...
		if($db->sql_numrows($result) != 0){
				...
				//Maintenant on regarde si commerce est dedans normalement il devrait pas mais on est jamais trop prudent...
