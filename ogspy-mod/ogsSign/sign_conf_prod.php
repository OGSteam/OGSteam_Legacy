...
if (empty($sign_exist)) {
	...
	if (!empty($pub_pseudoIG)) {
		...
		if (!empty($pub_choix_fond)) $query .= ', `nom_fond_prod`';	// car au début, aucun fond n'est sélectionné. Donc on mettra le fond par défaut prévu dans la structure de la base
		if (!empty($pub_couleur_txt_prod)) $query .= ', `couleur_txt_prod`';	// car au début, aucune couleur n'est inscrite.
		...
		// et on va supprimer la signature pour qu'elle se régénère ('P' pour purger uniquement la prod)
...
// modification des paramètres
	if (!empty($pub_pseudoIG) && !empty($pub_choix_fond)) {
		...
		// et on va supprimer la signature pour qu'elle se régénère ('P' pour purger la prod)
		// ne sert que dans le cas d'une réinstall
		vide_sign_cache('P', $pub_pseudoIG);
	}
}
...
if (!empty($param_sign['pseudo_ig'])) {
	$url_sign = 'mod/OGSign/'.$param_sign['pseudo_ig'].'.P.png';
...
// dossier des fonds
$dir_src_fonds = 'mod/OGSign/fonds/prod';
...
if (!$db->sql_numrows($db->sql_query($query))) {
	echo '<table width="100%" cellpadding="0" cellspacing="1"><tr><th><span style="color: red;">Le mod "Production" n\'est pas installé et/ou activé !</span><br />';
	echo 'La production inscrite sur cette signature ne reflètera pas réellement votre production dans le jeu !</th></tr></table>';
}
...
<!-- CHOIX DE LA FREQUENCE DE PROD -->
	<tr><td class="c" colspan="2">Choix de la fréquence de production</td></tr>
	<tr><th width="50%">Fréquence de la production affichée :</th>
	<th width="50%">
		...
		<label for="frequence_prod_hj">horaire et journalière</label>
		...
		<label for="frequence_prod_jh">journalière et hebdomadaire</label>
