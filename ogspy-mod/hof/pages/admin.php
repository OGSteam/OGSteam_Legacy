<?php
	...
	{		
		...
		$img		= (isset($_POST['img']) && $_POST['img'] == 'on') ? 1 : 0;
		$imgLink	= (isset($_POST['imgLink'])) ? mysql_real_escape_string (htmlentities ($_POST['imgLink'], ENT_QUOTES)) : 'http://uni3.ogame.fr/evolution/gebaeude/';
		...
		mysql_query('UPDATE '. TABLE_HOF_CONFIG .' SET value=\''. $img .'\' WHERE parameter=\'img\'');
		mysql_query('UPDATE '. TABLE_HOF_CONFIG .' SET value=\''. $imgLink .'\' WHERE parameter=\'imgLink\'');
...
<form action='index.php?action=hof&page=admin' method='post' class='admin'>
	...
	<table class='table_hof' style='width : 500px;'>
		...
		<tr>
			<td>Afficher les îcones</td>
			<td><input type='checkbox' name='img' <?php checked ($settings['img']); ?> /></td>
		</tr>
		<tr>
			<td>Lien des îcones<br />(0 => Îcones par defaut)</td>
			<td><input type='text' name='imgLink' size='30' value="<?php echo $settings['imgLink']; ?>" /></td>
		</tr>
	</table>
