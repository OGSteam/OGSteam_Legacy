...
if ($lune==1) {
	...
	foreach ($buildings_L as $key => $value) {
		if ($key != "DdR" || $ddr == 1) {
			...
			for ($i=1; $i<=$nbre_planete; $i++) {
				...
				$building_L_cumulate[$planete[$i]+9][$key] = intval($value["prix"] * (1 - pow($value["facteur"] , $user_building[$planete[$i]+9][$key])) / ((1 - $value["facteur"]) * 1000));
	...
	for ($i=1; $i<=$nbre_planete; $i++) {
		$building_L_cumulate[$planete[$i]+9]["SatL"] = 2.5 * $user_building[$planete[$i]+9]["Sat"];
...
foreach ($defenses_L as $key => $value) {
	...
	for ($i=1; $i<=$nbre_planete; $i++) {
		$user_defence[$planete[$i]+9][$key] = $defenses_L[$key]["prix"] * $user_defence[$planete[$i]+9][$key];
		if ($user_defence[$planete[$i]+9][$key] < "1") $affich = "&nbsp;";
		else $affich = number_format($user_defence[$planete[$i]+9][$key], 0, $virgule, $sep_mille);
		echo "\t<th><font color='lime'>".$affich."</font></th>\n";
		$user_defence_L_tot[$key] += $user_defence[$planete[$i]+9][$key];
	}
