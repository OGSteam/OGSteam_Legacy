...
if (!defined('IN_SPYOGAME')) die('hack');
...
if ($page == 'infos') {
	...
	if (isset($get_toolbar)) {
		$content = @file_get_contents('http://www.unibozu.fr/xtense/toolbar_info.txt');
		if ($content !== false) {
			$content = explode("\n", $content);
			$toolbar_data = array();
...
if ($page == 'config') {
	...
	if (isset($post_universe, $post_keep_log)) {
		...
		else $universe = 'http://uni0.ogame.fr';
	...
	if (isset($get_do)) {
		...
		if ($get_do == 'install_callbacks') {
			...
			$tpl->assign('total_callbacks', $total);
			$tpl->assign('installed_callbacks', $installed);
